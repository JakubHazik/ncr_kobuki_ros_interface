...
RobotInterface::RobotInterface(std::string ipAddress) {
...
void RobotInterface::t_readRobotData() {
    ...
    rob_si_posli.sin_addr.s_addr = inet_addr(ipAddress.c_str());
...
//    syslog(LOG_INFO, "ENCODER: left: %d; right: %d; gyro: %d", encoderLeft, encoderRight, gyroAngle );
    ...
    if (abs(encoderLeft - encoderLeftOld) > HWConfig::ENCODER_MAX / 2) {
        if (encoderLeft < encoderLeftOld) {
            encoderLeftDelta = HWConfig::ENCODER_MAX - encoderLeftOld + encoderLeft;
        } else {
            encoderLeftDelta = -(HWConfig::ENCODER_MAX - encoderLeft + encoderLeftOld);
        }
    } else {
    ...
    if (abs(encoderRight - encoderRightOld) > HWConfig::ENCODER_MAX / 2) {
        if (encoderRight < encoderRightOld) {
            encoderRightDelta = HWConfig::ENCODER_MAX - encoderRightOld + encoderRight;
        } else {
            encoderRightDelta = -(HWConfig::ENCODER_MAX - encoderRight + encoderRightOld);
        }
    } else {
        // v pripade bez pretecenia
    ...
    double distanceLeft = encoderLeftDelta * HWConfig::TICK_TO_METER;
    double distanceRight = encoderRightDelta * HWConfig::TICK_TO_METER;
    ...
    odom.fi = odom.fi + (distanceRight - distanceLeft) / HWConfig::WHEEL_BASE;
    ...
    if (distanceRight - distanceLeft == 0) {
        ...
        odom.x = odom.x + ((HWConfig::WHEEL_BASE * (distanceRight + distanceLeft)) / (2*(distanceRight - distanceLeft))) * (sin(odom.fi) - sin(fiOld));
        odom.y = odom.y - ((HWConfig::WHEEL_BASE * (distanceRight + distanceLeft)) / (2*(distanceRight - distanceLeft))) * (cos(odom.fi) - cos(fiOld));
    }
